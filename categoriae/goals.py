...
class Milestone(Goal):
    ...
    """
    target_date: Optional[datetime] = None  # Alias for clarity

    def __post_init__(self):
        """
        Post-initialization to set end_date from target_date.
        Milestones don't have start_date ranges, just a target date.
        """
        # If target_date was provided but end_date wasn't, sync them
        if self.target_date is not None and self.end_date is None:
            self.end_date = self.target_date
        # If end_date was provided but target_date wasn't, sync them
        elif self.end_date is not None and self.target_date is None:
            self.target_date = self.end_date
...
class SmartGoal(Goal):
    ...
    def __post_init__(self):
        ...
        if self.start_date is None or self.end_date is None:
            raise ValueError("SmartGoal requires both start_date and end_date (Time-bound)")
