...
class ActionGoalInferenceService:
    ...
    def infer_for_period(
        ...
        period_goals = [
            ...
            if self._goal_overlaps_period(g, start_date, end_date)
        ]
    ...
    def infer_for_new_action(
        ...
        if active_goals is None:
            ...
            active_goals = [
                g for g in all_goals
                if hasattr(g, 'end_date') and g.end_date and g.end_date >= datetime.now()
            ]
    ...
    def infer_for_goal(
        ...
        Args:
            ...
            end_date: Optional override for period end (uses goal.end_date if None)
        ...
        if end_date is None and hasattr(goal, 'end_date'):
        ...
        matches = infer_matches(
            ...
            goals=[goal],
            require_period_match=bool(start_date and end_date)
        )
    ...
    def _goal_overlaps_period(
        ...
        # Check for overlap: goal.start <= period.end AND goal.end >= period.start
        return goal.start_date <= end_date and goal.end_date >= start_date
